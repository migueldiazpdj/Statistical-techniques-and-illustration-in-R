##### Datos IGE ####
setwd(...)
 Datos.IGE <- read.delim("Datos-IGE.txt")
 View(Datos.IGE)
 ...
 #---- Lo que viene a continuaci?n es una prueba con factores para obtener una tabla como la de los apuntes.
 ...
 prop.test(x=sum(sabe.hablar.gallego=="SI"),
  ...
  #Recodificamos la variable tama?o municipio
  Gtamanomuni=rep(0,length(tamanomuni));Gtamanomuni
  Gtamanomuni[which(tamanomuni<=2)]=1 # Menos de 20000 hab
  
  # Estimamos la proporcion muestral en cada grupo
  exitos1=sum(sabe.hablar.gallego[Gtamanomuni==1]=="SI")
  exitos2=sum(sabe.hablar.gallego[Gtamanomuni==0]=="SI")
  pgorro1=exitos1/sum(Gtamanomuni==1);pgorro1
  pgorro2=exitos2/sum(Gtamanomuni==0);pgorro2

  # Realizamos el contraste
  prop.test(x=c(exitos1,exitos2),
            n=c(sum(Gtamanomuni==1),sum(Gtamanomuni==0)),
 ...
 t.test(conParkinson,sinParkinson,alternative="greater",paired=FALSE, conf.level=0.99)

 
 var.test(conParkinson,sinParkinson,alternative="two.sided",
